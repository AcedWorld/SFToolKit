...
namespace Unity.VisualScripting
{
	...
	{
		// Token: 0x060000F9 RID: 249 RVA: 0x0000407D File Offset: 0x0000227D
		public InvokeMember()
		{
		}

		// Token: 0x060000FA RID: 250 RVA: 0x00004085 File Offset: 0x00002285
		public InvokeMember(Member member) : base(member)
		{
		}

		// Token: 0x17000056 RID: 86
		// (get) Token: 0x060000FB RID: 251 RVA: 0x0000408E File Offset: 0x0000228E
		// (set) Token: 0x060000FC RID: 252 RVA: 0x00004096 File Offset: 0x00002296
		[Serialize]
		[InspectableIf("supportsChaining")]
		public bool chainable { get; set; }

		// Token: 0x17000057 RID: 87
		// (get) Token: 0x060000FD RID: 253 RVA: 0x0000409F File Offset: 0x0000229F
		[DoNotSerialize]
		...
		// (get) Token: 0x060000FE RID: 254 RVA: 0x000040AC File Offset: 0x000022AC
		// (set) Token: 0x060000FF RID: 255 RVA: 0x000040B4 File Offset: 0x000022B4
		[DoNotSerialize]
		...
		// (get) Token: 0x06000100 RID: 256 RVA: 0x000040BD File Offset: 0x000022BD
		// (set) Token: 0x06000101 RID: 257 RVA: 0x000040C5 File Offset: 0x000022C5
		[DoNotSerialize]
		[PortLabelHidden]
		public ControlInput enter { get; private set; }

		// Token: 0x1700005A RID: 90
		// (get) Token: 0x06000102 RID: 258 RVA: 0x000040CE File Offset: 0x000022CE
		// (set) Token: 0x06000103 RID: 259 RVA: 0x000040D6 File Offset: 0x000022D6
		[DoNotSerialize]
		public Dictionary<int, ValueInput> inputParameters { get; private set; }

		// Token: 0x1700005B RID: 91
		// (get) Token: 0x06000104 RID: 260 RVA: 0x000040DF File Offset: 0x000022DF
		// (set) Token: 0x06000105 RID: 261 RVA: 0x000040E7 File Offset: 0x000022E7
		[DoNotSerialize]
		[PortLabel("Target")]
		[PortLabelHidden]
		...
		// (get) Token: 0x06000106 RID: 262 RVA: 0x000040F0 File Offset: 0x000022F0
		// (set) Token: 0x06000107 RID: 263 RVA: 0x000040F8 File Offset: 0x000022F8
		[DoNotSerialize]
		[PortLabelHidden]
		public ValueOutput result { get; private set; }

		// Token: 0x1700005D RID: 93
		// (get) Token: 0x06000108 RID: 264 RVA: 0x00004101 File Offset: 0x00002301
		// (set) Token: 0x06000109 RID: 265 RVA: 0x00004109 File Offset: 0x00002309
		[DoNotSerialize]
		public Dictionary<int, ValueOutput> outputParameters { get; private set; }

		// Token: 0x1700005E RID: 94
		// (get) Token: 0x0600010A RID: 266 RVA: 0x00004112 File Offset: 0x00002312
		// (set) Token: 0x0600010B RID: 267 RVA: 0x0000411A File Offset: 0x0000231A
		[DoNotSerialize]
		[PortLabelHidden]
		public ControlOutput exit { get; private set; }

		// Token: 0x0600010C RID: 268 RVA: 0x00004124 File Offset: 0x00002324
		public override bool HandleDependencies()
		{
			...
			{
				if (base.member.parameterTypes.Length == base.defaultValues.Count((KeyValuePair<string, object> d) => d.Key != "target"))
				{
					this.parameterNames = (from d in base.defaultValues
					where d.Key != "target"
					select d into defaultValue
					select defaultValue.Key.Substring(1)).ToList<string>();
				}
			}
			return true;
		}

		// Token: 0x0600010D RID: 269 RVA: 0x000041DC File Offset: 0x000023DC
		protected override void Definition()
		{
			...
			for (int i = 0; i < this.parameterCount; i++)
			{
				...
				if (!parameterInfo.HasOutModifier())
				{
					...
					ValueInput valueInput = base.ValueInput(type, key);
				...
				if (parameterInfo.ParameterType.IsByRef || parameterInfo.IsOut)
				{
					...
					if (this.parameterNames != null && this.parameterNames[i] != parameterInfo.Name)
					{
						key2 = "&" + this.parameterNames[i];
					}
					ValueOutput valueOutput = base.ValueOutput(type, key2);
			...
			if (this.parameterNames == null)
			{
				this.parameterNames = (from pInfo in array
				select pInfo.Name).ToList<string>();
			}
		}

		// Token: 0x0600010E RID: 270 RVA: 0x000044D4 File Offset: 0x000026D4
		private void PostDeserializeRemapParameterNames()
		{
			...
			for (int j = 0; j < array.Length; j++)
			{
				...
				if (parameterInfo.Name != text)
				{
					...
					{
						ValueOutput[] array2 = (from con in valueInput.validConnections
						select con.source).ToArray<ValueOutput>();
						...
						object item;
						if (base.defaultValues.TryGetValue(valueInput.key, out item))
						{
							...
							list4.Add(new ValueTuple<string, object>("%" + parameterInfo.Name, item));
						}
					}
					else if (base.valueOutputs.TryGetValue("&" + text, out valueOutput))
					{
						ValueInput[] array4 = (from con in valueOutput.validConnections
						select con.destination).ToArray<ValueInput>();
		...
		// Token: 0x06000110 RID: 272 RVA: 0x000048B0 File Offset: 0x00002AB0
		private object Invoke(object target, Flow flow)
		{
			if (!this.useExpandedParameters)
			{
				...
				{
					ValueInput input;
					if (this.inputParameters.TryGetValue(i, out input))
					{
						array[i] = flow.GetConvertedValue(input);
					}
				}
				object result = base.member.Invoke(target, array);
				for (int j = 0; j < this.parameterCount; j++)
				{
					ValueOutput port;
					if (this.outputParameters.TryGetValue(j, out port))
					{
						flow.SetValue(port, array[j]);
					}
				}
				return result;
			}
		...
		// Token: 0x06000111 RID: 273 RVA: 0x00004AD4 File Offset: 0x00002CD4
		...
		// Token: 0x06000112 RID: 274 RVA: 0x00004B28 File Offset: 0x00002D28
		...
		// Token: 0x06000113 RID: 275 RVA: 0x00004B48 File Offset: 0x00002D48
		private ControlOutput Enter(Flow flow)
		{
			object andChainTarget = this.GetAndChainTarget(flow);
			object value = this.Invoke(andChainTarget, flow);
			if (this.result != null)
			{
				flow.SetValue(this.result, value);
			}
			return this.exit;
		}

		// Token: 0x06000114 RID: 276 RVA: 0x00004B84 File Offset: 0x00002D84
