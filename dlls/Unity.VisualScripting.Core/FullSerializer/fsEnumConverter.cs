...
namespace Unity.VisualScripting.FullSerializer
{
	...
	{
		// Token: 0x06000A21 RID: 2593 RVA: 0x0002A42D File Offset: 0x0002862D
		public override bool CanProcess(Type type)
		{
			return type.Resolve().IsEnum;
		}

		// Token: 0x06000A22 RID: 2594 RVA: 0x0002A43A File Offset: 0x0002863A
		public override bool RequestCycleSupport(Type storageType)
		{
			return false;
		}

		// Token: 0x06000A23 RID: 2595 RVA: 0x0002A43D File Offset: 0x0002863D
		public override bool RequestInheritanceSupport(Type storageType)
		{
			return false;
		}

		// Token: 0x06000A24 RID: 2596 RVA: 0x0002A440 File Offset: 0x00028640
		public override object CreateInstance(fsData data, Type storageType)
		{
			return Enum.ToObject(storageType, 0);
		}

		// Token: 0x06000A25 RID: 2597 RVA: 0x0002A450 File Offset: 0x00028650
		...
		// Token: 0x06000A26 RID: 2598 RVA: 0x0002A538 File Offset: 0x00028738
			...
			if (data.IsString)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					if (!fsEnumConverter.ArrayContains<string>(Enum.GetNames(storageType), text))
					{
						...
						select new ValueTuple<Enum, string>(x, attr.previousName)).ToDictionary(([TupleElementNames(new string[]
						{
							"enumMember",
							"previousName"
						})] ValueTuple<Enum, string> x) => x.Item2, ([TupleElementNames(new string[]
						{
							"enumMember",
							"previousName"
						})] ValueTuple<Enum, string> x) => x.Item1).TryGetValue(text, out @enum))
						{
		...
		// Token: 0x06000A27 RID: 2599 RVA: 0x0002A73C File Offset: 0x0002893C
