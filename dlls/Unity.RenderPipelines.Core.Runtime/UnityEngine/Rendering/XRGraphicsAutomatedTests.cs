...
namespace UnityEngine.Rendering
{
	...
	public static class XRGraphicsAutomatedTests
	{
		...
		// (get) Token: 0x06000873 RID: 2163 RVA: 0x000277B5 File Offset: 0x000259B5
		...
		// (get) Token: 0x06000874 RID: 2164 RVA: 0x000277B8 File Offset: 0x000259B8
		public static bool enabled { get; } = XRGraphicsAutomatedTests.activatedFromCommandLine;

		// Token: 0x06000875 RID: 2165 RVA: 0x000277C0 File Offset: 0x000259C0
				...
				if (camera.TryGetCullingParameters(false, out cullingParams))
				{
					...
					for (int i = 0; i < activePasses.Count; i++)
					{
						...
						for (int j = 0; j < item.viewCount; j++)
						{
							...
							XRView xrView = new XRView(projMatrix, matrix4x, item.GetViewport(j), null, item.GetTextureArraySlice(j));
