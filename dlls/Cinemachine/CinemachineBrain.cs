...
namespace Cinemachine
{
	...
	public class CinemachineBrain : MonoBehaviour, ICameraOverrideStack
	{
		...
		private void UpdateFrame0(float deltaTime)
		{
			...
			{
				if ((Object)cinemachineCamera != null && (Object)camB != null && deltaTime >= 0f)
				{
			...
			if ((state.BlendHint & CameraState.BlendHintValue.NoLens) == CameraState.BlendHintValue.Nothing)
			{
				...
				if (outputCamera != null)
				{
					...
					if (flag && state.Lens.IsPhysicalCamera)
					{
						...
						outputCamera.iso = state.Lens.Iso;
						outputCamera.shutterSpeed = state.Lens.ShutterSpeed;
						outputCamera.aperture = state.Lens.Aperture;
						outputCamera.bladeCount = state.Lens.BladeCount;
						outputCamera.curvature = state.Lens.Curvature;
						outputCamera.barrelClipping = state.Lens.BarrelClipping;
						outputCamera.anamorphism = state.Lens.Anamorphism;
					}
				}
			}
		...
		private class BrainFrame
		{
			...
			// (get) Token: 0x06000408 RID: 1032 RVA: 0x0001880E File Offset: 0x00016A0E
			public bool Active
			{
				get
				{
